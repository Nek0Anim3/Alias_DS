...
async def lobbies_list_kb(lobbies_list: list):
    ...
    for count in range (0, listsize):
        ...
        print("Button id ", list_id[count])
        builder.button(text=f"Lobby {lobbies_list[count]}", callback_data=f"join_lobby:{list_id[count][0]}")#LobbyCallback(action="join", lobby_id=str(list_id[count])))
